...
void* create_aichat_app_ex(const char* address, int port, const char* token, const char* deviceId, const char* aliyun_api_key, int protocolVersion, int sample_rate, int channels, int frame_duration, const char* vision_model_path, const char* face_model_path) {
    auto* app = new Application(std::string(address), port, std::string(token), std::string(deviceId), std::string(aliyun_api_key), protocolVersion, sample_rate, channels, frame_duration, std::string(vision_model_path), std::string(face_model_path ? face_model_path : ""));
    return static_cast<void*>(app);
}
